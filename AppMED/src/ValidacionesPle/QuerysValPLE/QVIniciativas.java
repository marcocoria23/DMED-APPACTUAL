...
"ORDER BY ID_INICIATIVA,AÑO_LEGISLATURa)";
      ...
      try {
          while (resul.next()) {
              Array.add(new String[]{
                  resul.getString("ID_ENTIDAD"),
                  resul.getString("AÑO_LEGISLATURA"),
                  resul.getString("ID_INICIATIVA"),
                  resul.getString("ESTATUS_INICIATIVA"),
                  resul.getString("ETAPA_PROCESAL_INICIATIVALLEGISLA"),
...
public ArrayList Iniciativas_Duplicadas(String ID_entidad,String Legislatura,String envio,String envio_anterior){
     conexion.Conectar();
      Array = new ArrayList();
      sql="select * from(  \n" +
"select count(ID_ACTUAL) as conteo, ID_ACTUAL, ESTATUS_ACTUAL from( \n" +
      ...
      try {
          while (resul.next()) {
              Array.add(new String[]{
                  resul.getString("CONTEO"),
                  resul.getString("ID_ACTUAL"),
                  resul.getString("ESTATUS_ACTUAL")
                });
          }
      conexion.close();
     } catch (SQLException ex) {
...
public ArrayList Iniciativas_Duplicada_presentaPeriodo(String ID_entidad,String Legislatura,String envio,String envio_anterior){
     conexion.Conectar();
      Array = new ArrayList();
      sql="select count(*) as CONTEO, P1_5_1 AS ID_INICIATIVA, P1_5_3 AS COND_PRESENTACION_INICIATIVA_PERIODO\n" +
"FROM TR_PLE_MEDS1_5\n" +
"where P1_5_3 =1 and entidad='"+ID_entidad+"'\n" +
"group by P1_5_1,P1_5_3\n" +
"having count(*) > 1" ;
         System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
