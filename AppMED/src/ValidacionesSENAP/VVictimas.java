...
public class VVictimas {
    ...
    public ArrayList EDADMENOR1(String Entidad, String Periodo) throws SQLException {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT * FROM TR_SENAP_VICTIMAS\n" +
"WHERE EDADMOMSUCHEC < -1  AND (ENTIDADID='" + Entidad + "' AND PERIODO='" + Periodo + "')";
    ...
    public ArrayList EDADHECHOS(String Entidad, String Periodo) throws SQLException {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT VICTIMAID,EDADMOMSUCHEC FROM TR_SENAP_VICTIMAS\n" +
"WHERE EDADMOMSUCHEC>106 and (ENTIDADID='" + Entidad + "' AND PERIODO='" + Periodo + "')";
    ...
    //ReglaNegocio Adicional: La relación entre los campos  CatEntidadFederativaID (Entidad) y CATMUNICIPIOID (Municipio) de la plantilla Victimas, no coinciden con la relación de los campos CATENTIDADFEDERATIVAID y CATMUNICIPIODEMARCACIONTERRITORIALID del catálogo SENAP_MUNICIPIODEMARCACIONTERRITOR
...
"where catalogo is null  AND (ENTIDADID='" + Entidad + "' AND PERIODO='" + Periodo + "') and  CATSUBPROREFID=5";
    ...
    //ReglaNegocio Adicional: La relación entre los campos  CatEntidadFederativa(Entidad DE NACIMIENTO) y CATMUNICIPIOID (Municipio DE NACIMIENTO) de la plantilla Victimas, no coinciden con la relación de los campos CATENTIDADFEDERATIVAID y CATMUNICIPIODEMARCACIONTERRITORIALID del catálogo SENAP_MUNICIPIODEMARCACIONTERRITOR
public ArrayList RelacionMunicipioNacimiento(String Entidad, String Periodo) throws SQLException {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "select * from (\n" +
"select victimaID, CARPETAID,DIRECCIONUBICACIONID,CATMUNICIPIOID, CATENTIDADNACIMIENTOID, tc.CATMUNICIPIODEMARCACIONTERRITORIALID as catalogo, vic.periodo, vic.ENTIDADID from TR_SENAP_VICTIMAS vic\n" +
...
"where catalogo is null and (ENTIDADID='" + Entidad + "' AND PERIODO='" + Periodo + "')";
    ...
    //Si en el campo Tipo de víctima (columna F), es requisitado con el valor “Persona moral del sector público, Persona moral del sector privado o Persona moral no identificado”, se deberá permitir únicamente el registro de los campos ¿Contó con asesor jurídico? (columna AY) y Tipo de asesor jurídico (columna AZ) no podran venir vacios
...
"where CATTIPOVICTIMAID in (2,3,4) and (CONTOASESORJURIDICO is null or CATTIPOASESORJURIDICOID is null) AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
        ...
        //Si el campo Tipo de víctima corresponde a Persona física (1), deberá estar inscrita información en todos los campos de esta plantilla.
...
"CARPETAID_FISCALIA is null ) AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
        ...
        //Si el campo ¿Habla lengua extranjera? (columna AG) es requisitado con la opción “Si”, el campo Tipo de lengua extranjera (columna AH) no debera de venir vacia.
...
"from tr_senap_victimas\n" +
"where HABLALENGUAEXTRANJERA=1 and CATTIPLEGEXT is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
      System.out.println(sql);
      Query=sql;
      resul=conexion.consultar(sql);
      try {
    ...
    return Array;
            ...
            //Si el campo ¿Habla lengua extranjera? (columna AG) es diferente de la opción “Si”, el campo Tipo de lengua extranjera (columna AH) no debera de venir con valor.
...
"where HABLALENGUAEXTRANJERA<>1 and CATTIPLEGEXT is  not null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    //Si el campo ¿Habla lengua indígena? (columna AI) es requisitado con la opción “Si”, el campo Tipo de lengua indígena (columna AJ) no debera venir vacio.
...
"where HABLALENGUAINDIGENA=1 and CATTIPOLENGUAINDIGENAID is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
        ...
        //Si el campo ¿Habla lengua indígena? (columna AI) es diferente de la opción “Si”, el campo Tipo de lengua indígena (columna AJ) no debera venir con valor.
...
"where HABLALENGUAINDIGENA<>1 and CATTIPOLENGUAINDIGENAID is  not null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
            ...
            //Si el campo ¿Presenta algún tipo de discapacidad? (columna AL) es requisitado con la opción “Si”, los campos Condición de discapacidad (columna AM) y ¿Utilizó intérprete y/o medio tecnológico por discapacidad? (columna AN) no pueden venir vacios
...
"where PRESENTADISCAPACIDAD=1 and (CATTIPODISCAPACIDADID is null or MEDIOTECDISCAPACIDAD is null) AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
     ...
     //Si el campo ¿Presenta algún tipo de discapacidad? (columna AL) es diferente de la opción “Si”, los campos Condición de discapacidad (columna AM) y ¿Utilizó intérprete y/o medio tecnológico por discapacidad? (columna AN) no pueden venir con valor
...
"where PRESENTADISCAPACIDAD<>1 and (CATTIPODISCAPACIDADID is not null or MEDIOTECDISCAPACIDAD is not null) AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
         ...
         //Si el campo ¿Pertenece a alguna población indígena? (columna AO) es requisitado con la opción “Si”, el campo Población indígena a la que pertenece (columna AP) no puede venir vacio.
...
"where PERPOPIND=1 and CATTIPPOPIND is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    //Si el campo ¿Pertenece a alguna población indígena? (columna AO) es diferente de la opción “Si”, el campo Población indígena a la que pertenece (columna AP) no puede venir con valor.
...
"where PERPOPIND<>1 and CATTIPPOPIND is not null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
        ...
        //Si el campo ¿Pertenece a alguna población LGBTTTI? (columna AQ) es requisitado con la opción “Si”, el campo Población LGBTTTI a la que pertenece (columna AR) no podra venir vacio.
...
"where PERPOBLGBTTTI=1 and CATTIPOLGBTTTIID is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    //Si el campo ¿Pertenece a alguna población LGBTTTI? (columna AQ) es diferente de  la opción “Si”, el campo Población LGBTTTI a la que pertenece (columna AR) no puede traer valor.
...
"where PERPOBLGBTTTI<>1 and CATTIPOLGBTTTIID is not null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    //Si el campo ¿Contó con asesor jurídico?  (columna AY) es requisitado con la opción “Si”, el campo Tipo de asesor jurídico (columna AZ) no podra venir vacio.
...
"where CONTOASESORJURIDICO=1 and CATTIPOASESORJURIDICOID is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    //Si el campo ¿Contó con asesor jurídico?  (columna AY) es diferente de la opción “Si”, el campo Tipo de asesor jurídico (columna AZ) no podra venir con valor.
...
"where CONTOASESORJURIDICO<>1 and CATTIPOASESORJURIDICOID is not null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    //ReglaNegocio GENERAL. La FECHANACIMIENTO (Fecha de nacimiento) no debe ser mayor a la fecha Actual 
...
"WHERE FECHANACIMIENTO > SYSDATE  AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
    ...
    return Array;
     ...
     //ReglaNegocio 6.3. Favor de verificar la FECHANACIMIENTO (Fecha de nacimiento) ya que es mayor a 109 años de Edad
...
"EXTRACT (YEAR FROM FECHANACIMIENTO)  as años_actual,FECHANACIMIENTO \n" +
"FROM \n" +
"tr_senap_victimas) where EDADMOMSUCHEC>años_actual AND (FECHANACIMIENTO<>'01/01/1900')  AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"'  )";
      System.out.println(sql);
      Query=sql;
      resul=conexion.consultar(sql);
      try {
...
//Regla Adicional -- Favor de verificar el campo Tipo de víctima CATTIPOVICTIMAID debido a que no se puede seleccionar una opción diferente a "Persona física" (1), "Persona moral del sector público" (2), "Persona moral del sector privado" (3), "Persona moral no identificado" (4), "Otro" (5) y "No identificado" (6).
...
//Regla Adicional -- Favor de verificar el campo Edad al momento que sucedieron los hechos	EDADMOMSUCHEC debido que a que no puede venir nulo.
