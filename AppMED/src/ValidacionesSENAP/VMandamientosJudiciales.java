...
public class VMandamientosJudiciales {
    ...
    //El campo Fecha de solicitud del mandamiento judicial (columna E) debe ser mayor o igual al campo Fecha de apertura de la carpeta de investigación (columna X) de la plantilla Carpeta de investigación.
...
"where MJ.FECHASOLMANDJUD < CI.FECHACARINVE AND (MJ.FECHASOLMANDJUD<>'01/01/1900') and (CI.FECHACARINVE<>'01/01/1900') AND (MJ.ENTIDADID='"+Entidad+"' AND MJ.PERIODO='"+Periodo+"')";
    ...
    //El campo Fecha de libramiento del mandamiento (columna G) debe ser mayor o igual al campo Fecha de ingreso de la causa penal (columna F) de la plantilla Proceso.
...
"where MJ.FECHALIBMAND < P.FECHAINICIO AND (MJ.FECHALIBMAND<>'01/01/1900') and (P.FECHAINICIO<>'01/01/1900') AND (MJ.ENTIDADID='"+Entidad+"' AND MJ.PERIODO='"+Periodo+"')";
      System.out.println(sql);
      Query=sql;
      resul=conexion.consultar(sql);
      try {
    ...
    //El campo Estatus del mandamiento judicial (columna H) es requisitado con la opcion “Cumplimentado” (1) debe ser requisitado el campo Fecha de cumplimento del mandamiento judicial (columna I).
...
"and FECHAULTESTMAND is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //El campo Fecha de cumplimento del mandamiento (columna I) debe ser mayor o igual al campo Fecha de libramiento del mandamiento (columna G).
...
"where FECHAULTESTMAND < FECHALIBMAND AND (FECHAULTESTMAND<>'01/01/1900') and (FECHALIBMAND<>'01/01/1900') AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //El campo Estatus del mandamiento judicial (columna H) es requisitado con la opción “Cumplimentado” (1) debe ser requisitado el campo Fecha de cumplimento del mandamiento judicial (columna I).
...
"and FECHAULTESTMAND is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //La FECHASOLMANDJUD (Fecha de solicitud del mandamiento judicial ) No debe de ser mayor a la fecha actual.
...
"WHERE FECHASOLMANDJUD > SYSDATE AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    return Array;
     ...
     //La FECHALIBMAND (Fecha de libramiento del mandamiento) No debe de ser mayor a la fecha actual.
...
"WHERE FECHALIBMAND > SYSDATE AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    return Array;
     ...
     //La FECHAULTESTMAND (Fecha de libramiento del mandamiento) No debe de ser mayor a la fecha actual.
