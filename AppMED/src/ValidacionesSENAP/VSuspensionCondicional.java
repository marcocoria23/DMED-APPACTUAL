...
public class VSuspensionCondicional {
    ...
    //La plantilla Suspensión condicional es requisitada, el campo Fecha en que se cumplimentó la suspensión condicional del proceso (columna J) debe ser requisitado y las plantillas MASC y Sentencia no deben ser requisitadas.
...
"	or S.TABLA_SENTENCIA is not null) AND (SC.ENTIDADID='"+Entidad+"' AND SC.PERIODO='"+Periodo+"')";
    ...
    //El campo Fecha en que dictó el sobreseimiento (columna G) de la plantilla Sobreseimiento debe ser mayor al campo Fecha en que se cumplimentó la suspensión condicional del proceso (columna J).
...
"where SB.FECHADICTOSOBRESEIMIENTO <= SC.FECHACUMPSUSPENCONDIPROCESO  AND (SB.FECHADICTOSOBRESEIMIENTO<>'01/01/1900') and (SC.FECHACUMPSUSPENCONDIPROCESO<>'01/01/1900') AND (SC.ENTIDADID='"+Entidad+"' AND SC.PERIODO='"+Periodo+"')";
    ...
    //El campo Etapa en la que se dictó  la suspensión condicional del proceso es requisitado con la opción "Etapa intermedia" (1) entonces el campo Etapa procesal en la que se encuentra el imputado debe ser igual a "Etapa intermedia" (3) y el campo ¿Se dictó auto de apertura a juicio oral? (columna K) debe ser requisitado con la opción "No" (2).
...
"and EI.DICTAUTAPERJUICIO <> 2 AND (SC.ENTIDADID='"+Entidad+"' AND SC.PERIODO='"+Periodo+"')";
    ...
    //En la plantilla Suspensión condicional existen registros, no debe haber registros en las plantillas MASC y Sobreseimiento
...
"and SB.TABLA_SOBRESEIMIENTO is not null AND (SC.ENTIDADID='"+Entidad+"' AND SC.PERIODO='"+Periodo+"')";
    ...
    //El campo Fecha en que se dictó la suspensión condicional del proceso (columna E) es requisitado, debe ser mayor o igual al campo Fecha en que se dictó el auto de vinculación a proceso (columna G) de la plantilla Investigación complementaria.
...
"and FECHAREAPERTURAPROCESO <= FECHADICSUSCONPRO AND (FECHAREAPERTURAPROCESO<>'01/01/1900') and (FECHADICSUSCONPRO<>'01/01/1900') AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //El campo ¿Hubo reapertura del proceso? (columna H) es requisitado con la opción “Si” (1) entonces el campo Fecha de reapertura del proceso (columna I) debe ser requisitado.
...
"and FECHAREAPERTURAPROCESO is null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //El campo ¿Hubo reapertura del proceso? (columna H) es diferente a la opción “Si” (1) entonces el campo Fecha de reapertura del proceso (columna I) no debe ser requisitado.
...
"and FECHAREAPERTURAPROCESO is not null AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //El campo Fecha en que se cumplimentó la suspensión condicional del proceso (columna J) debe ser mayor o igual al campo Fecha en que se dictó la suspensión condicional del proceso (columna E).
...
"where FECHACUMPSUSPENCONDIPROCESO < FECHADICSUSCONPRO AND (FECHACUMPSUSPENCONDIPROCESO<>'01/01/1900') and (FECHADICSUSCONPRO<>'01/01/1900') AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //El campo ¿Hubo reapertura del proceso? (columna H) es requisitado con la opción “Si” (1), entonces el campo Fecha en que se cumplimentó la suspensión condicional del proceso (columna J) no debe ser requerido y el campo Fecha de reapertura del proceso (columna I) debe ser requisitado.
...
"	or FECHAREAPERTURAPROCESO is null)  AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    //La FECHADICSUSCONPRO (Indica la etapa del proceso penal en la que se decretó la suspensión condicional del proceso. ) No debe de ser mayor a la fecha actual.
...
//La FECHAREAPERTURAPROCESO () No debe de ser mayor a la fecha actual.
...
"WHERE FECHAREAPERTURAPROCESO > SYSDATE AND (ENTIDADID='"+Entidad+"' AND PERIODO='"+Periodo+"')";
    ...
    return Array;
     ...
     //La FECHACUMPSUSPENCONDIPROCESO () No debe de ser mayor a la fecha actual.
