...
public class VImputado {
    ...
    //ReglaNegocio Adicional EDADMOMENTOSUCEDIERONHECHOS-(Edad al momento que sucedieron los hechos) No debe ser Menor a -1
    ...
    //ReglaAdicional 7.3. Favor de verificar el campo  Edad al momento que sucedieron los hechos (columna K) EDADMOMENTOSUCEDIERONHECHOS debido que no pueden registrar con valor cero o nulo.
    ...
    //ReglaNegocio Adicional: La relación entre los campos  CatEntidadFederativa(Entidad DOMICILIO) y CATMUNICIPIOID (Municipio DOMICILIO) de la plantilla Imputado, no coinciden con la relación de los campos CATENTIDADFEDERATIVAID y CATMUNICIPIODEMARCACIONTERRITORIALID del catálogo SENAP_MUNICIPIODEMARCACIONTERRITOR
    ...
    //ReglaNegocio Adicional: La relación entre los campos  CatEntidadFederativa(Entidad OCURRENCIA DEL DELITO) y CATMUNICIPIOID (Municipio OCURRENCIA DEL DELITO) de la plantilla Imputado, no coinciden con la relación de los campos CATENTIDADFEDERATIVAID y CATMUNICIPIODEMARCACIONTERRITORIALID del catálogo SENAP_MUNICIPIODEMARCACIONTERRITOR
    ...
    //ReglaNegocio Adicional: La relación entre los campos  CatEntidadFederativa(Entidad DE NACIMIENTO) y CATMUNICIPIOID (Municipio DE NACIMIENTO) de la plantilla Imputado, no coinciden con la relación de los campos CATENTIDADFEDERATIVAID y CATMUNICIPIODEMARCACIONTERRITORIALID del catálogo SENAP_MUNICIPIODEMARCACIONTERRITOR
public ArrayList RelacionMunicipioNacimiento(String Entidad, String Periodo) throws SQLException {
    ...
    //ReglaNegocio Adicional favor de verificar EDADMOMENTOSUCEDIERONHECHOS-Edad al momento que sucedieron los hechos ya qye es mayor a 106 años
    ...
    //Si en el campo Tipo de persona imputada (columna G), es seleccionada la opción “Persona física”, deberá estar inscrita información en todos los campos de esta plantilla, con excepción del campo, Tipo de persona moral (columna H)
    ...
    //Si en el campo Tipo de persona imputada (columna G), es seleccionadaa la opción “Persona física”, deberá estar inscrita información en todos los campos de esta plantilla, Tipo de persona moral (columna H) debe de estar vacío
    ...
    //Si en el campo Tipo de persona imputada (columna G), es seleccionada la opción “Persona moral”, se deberá permitir el registro sólo de los campos Tipo de persona moral (columna H), ¿Contó con persona defensora? (columna BE  ) y Tipo de persona defensora (columna BF)  
    ...
    //Si en el campo ¿Habla lengua extranjera? (columna AI) es seleccionada la opción “Sí”, el campo Tipo de lengua extranjera (columna AJ) no debe de estar vacío
    ...
    //Si el campo ¿Habla lengua extranjera? (columna AI) es diferente de la opción “Sí”, el campo Tipo de lengua extranjera (columna AJ) no debe de contener información
    ...
    //Si en el campo ¿Habla lengua indígena? (columna AK) es selecconada la opción “Sí”, el campo Tipo de lengua indígena (columna AL) no puede venir vacio
    ...
    //Si en el campo ¿Habla lengua indígena? (columna AK) es selecconada la opción “Sí”, el campo Tipo de lengua indígena (columna AL) no deberá venir con valor
    ...
    //Si en el campo ¿Presenta alguna condición de discapacidad? (columna AN) es selecconada la opción “Sí”,  los campos Condición de discapacidad (columna AO) y ¿Utilizó intérprete y/o medio tecnológico por discapacidad? (columna AP) no pueden estar vacíos.
    ...
    //Si el campo ¿Presenta alguna condición de discapacidad? (columna AN) es diferente a la opción “SÍ”,  los campos Condición de discapacidad (columna AO) y ¿Utilizó intérprete y/o medio tecnológico por discapacidad? (columna AP) no deberán contener información
    ...
    //Si en el campo ¿Pertenece a alguna población indígena? (columna AQ) es seleccionada la opción “Sí”, el campo Población indígena a la que pertenece (columna AR) no puede estar vacío
    ...
    //Si el campo ¿Pertenece a alguna población indígena? (columna AQ) es diferente a la opción “SÍ”, el campo Población indígena a la que pertenece (columna AR) no puede contener información
    ...
    //Si en el campo ¿El imputado manifestó pertenecer a un probable grupo delictivo? (columna AZ) es seleccionada la opción “Sí”, el campo Nombre del grupo al que pertenece (columna BA) no puede estar vacío
    ...
    //Si el campo ¿El imputado manifestó pertenecer a un probable grupo delictivo? (columna AZ) es diferente de la opción “Si”, el campo Nombre del grupo al que pertenece (columna BA) no puede contener información
    ...
    //-----campo:    ContoDefensor ----------------***
    //Si en el campo ¿Contó con persona defensora? (columna BE)  es seleccionada la opción “Sí”, el campo Tipo de defensor (columna BF) no puede estar vacío
    ...
    //Si el campo ¿Contó con persona defensora? (columna BE) es diferente de la opción “Sí”, el campo Tipo de defensor (columna BF) no puede contener información
    ...
    //ReglaNegocio GENERAL. La Fecha de audiencia de control de la detención (columna BY) deberá ser mayor o igual a la Fecha de apertura de la carpeta de investigación (columna X) de la plantilla Carpeta de investigación
    ...
    //La FECHANACIMIENTO (Fecha de nacimiento) no debe ser mayor a la fecha Actual 
    ...
    //ReglaAdicional GENERAL. Favor de verificar la FECHANACIMIENTO (Fecha de nacimiento) ya que es mayor a 109 años de Edad
    ...
    //Regla Adicional -- Favor de verificar el campo Fecha de nacimiento	FECHANACIMIENTO debido que a que no puede venir nulo.
    ...
    //Regla Adicional -- Favor de verificar el campo Edad al momento que sucedieron los hechos	EDADMOMENTOSUCEDIERONHECHOS debido que a que no puede venir nulo.
    public ArrayList Edadnotnull(String Entidad, String Periodo) throws SQLException {
        conexion.Conectar();
        Array = new ArrayList();
...
//Regla Adicional -- Favor de verificar el campo Sexo	CATSEXOID debido a que no se puede seleccionar una opción diferente a "Hombre" (1), "Mujer" (2) y "No identificado" (3).
    ...
    //Regla Adicional -- Favor de verificar el campo Situación conyugal	CATSITUACIONCONYUGALID debido a que no se puede seleccionar una opción diferente a "Casado(a)" (1), "Unión Libre" (2), "Divorciado(a)" (3), "Separado(a)" (4), "Soltero(a)" (5), "Concubinato" (6), "Sociedad de convivencia" (7), "Viudo(a)" (8), "Otro" (9) y "No identificado" (10).
...
//Regla Adicional -- Favor de verificar el campo Nacionalidad CATNACIONALIDADID debido a que no se puede venir nulo
...
//Regla Adicional -- Favor de verificar el campo Nivel de escolaridad	CATNIVELESCOLARIDADID debido a que no se puede seleccionar una opción diferente a "Ninguno" (1), "Preescolar o primaria" (2), "Secundaria" (3), "Preparatoria" (4), "Carrera técnica o comercial" (5), "Licenciatura" (6), "Maestría" (7), "Doctorado" (8) y "No identificado" (9).
...
//Regla Adicional -- Favor de verificar el campo Antecedentes penales	ANTECEDENTESPENALES debido a que no se puede seleccionar una opción diferente a "Si" (1), "No" (2) y "No identificado" (3).
    ...
    //Regla Adicional -- Favor de verificar el campo Tipo de persona imputada	CATTIPOPERSONAIMPUTADAID debido a que no se puede seleccionar una opción diferente a "Persona física" (1), "Persona moral" (2) y "No identificado" (3).
