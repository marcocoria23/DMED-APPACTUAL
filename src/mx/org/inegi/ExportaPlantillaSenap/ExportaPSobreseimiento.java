 ...
 */
public class ExportaPSobreseimiento {

    private ConOracle conexion = new ConOracle();
    private ResultSet resul;

    public void exportaPSobreseimiento(String Entidad, String Periodo, String csvFile) throws SQLException {
        ...
        String sql = "SELECT \n"
                ...
                + "IMPUTADOID, \n"
                + "PROCESOID,\n"
                + "CONVERI_ETAPASOBRESEIMIENTO(CATETAPASOBRESEIMIENTOID)CATETAPASOBRESEIMIENTOID,\n"
                + "CONVERI_CAUSASSOBRESEIMIENTO(CATCAUSASSOBRESEIMIENTOID)CATCAUSASSOBRESEIMIENTOID,\n"
                + "TO_CHAR(FECHADICTOSOBRESEIMIENTO,'D/MM/YYYY')FECHADICTOSOBRESEIMIENTO,\n"
                + "CATTIPOSOBRESEIMIENTOID,\n"
                + "CONVERI_TIPOSOBRESEIMIENTO(SOBRESEIMIENTOID_FISCALIA)SOBRESEIMIENTOID_FISCALIA,\n"
                + "PERIODO\n"
                + "FROM\n"
                + "TR_SENAP_SOBRESEIMIENTO"
                + " WHERE (ENTIDADID='" + Entidad + "' and PERIODO='" + Periodo + "')";
        resul = conexion.consultar(sql);

        Vector<String> encabezados = new Vector<>();
        encabezados.add("ENTIDAD");
        encabezados.add("SobreseimientoID ");
        encabezados.add("CarpetaID");
        encabezados.add("ImputadoID ");
        encabezados.add("ProcesoID");
        encabezados.add("En caso de sobreseimiento, etapa en la que se dictó el sobreseimiento");
        encabezados.add("Causas de sobreseimiento ");
        encabezados.add("Fecha en que se dictó el sobreseimiento");
        encabezados.add("Tipo de sobreseimiento ");
