...
public class VNoticiaCriminal {
    ...
    //ReglaDMED. La relación entre los campos  CatEntidadFederativaID (Entidad) y CATMUNICIPIOID (Municipio) de la tabla Noticia Criminal, no coinciden con la relación de los campos CATENTIDADFEDERATIVAID y CATMUNICIPIODEMARCACIONTERRITORIALID del catálogo  SENAP_MUNICIPIODEMARCACIONTERRITOR
    ...
    //ReglaDMED. El campo Fecha de presentación de la denuncia o querella (columna U) no puede ser mayor a la Fecha Actual.
    ...
    //ReglaNegocio 1.2. El campo Fecha de conclusión de la atención FECHCONCLUATE (TR_SENAP_ATENCIONES) deberá ser mayor al campo Fecha de inicio de la atención FECHAINICIOATENCION (TR_SENAP_ATENCIONES).
    ...
    //ReglaAdicional 1.3. Si el campo "Sentido de la conclusión de la atención" es requisitado con la opción "Apertura de carpeta de investigación" (1) se deberá continuar con el registro de la siguiente plantilla Carpeta de investigación.
    public ArrayList CATSENTCONCLUATEID(String Entidad, String Periodo) throws SQLException {
        ...
        sql = "SELECT ATE.ENTIDADID, ATE.PERIODO, ATE.ATENCIONID, ATE.NOTICIACRIMINALID, ATE.CATSENTCONCLUATEID, CI.TABLA_CARPETAINVESTIGACION\n"
                ...
                + "AND CI.TABLA_CARPETAINVESTIGACION IS NULL AND (ATE.ENTIDADID='" + Entidad + "' AND ATE.PERIODO='" + Periodo + "'  )";
        System.out.println(sql);
        Query = sql;
        resul = conexion.consultar(sql);
        try {
    ...
    //ReglaAdicional 1.3.1. Si el campo "Sentido de la conclusión de la atención" es diferente de la opción "Apertura de carpeta de investigación" (1) no debe existir registro en la plantilla Carpeta de investigación.
    ...
    //ReglaAdicional 1.3.2. Si el campo "Sentido de la conclusión de la atención" se selecciona la opción "Conclusión" (2), "Vista a otra autoridad por incompetencia" (3) u "Otro" (4) se dará por concluido el registro y no se permitirá el llenado de ninguna otra plantilla.
    ...
    //ReglaAdicional 1.3.3. Si el campo "Sentido de la conclusión de la atención" se selecciona la opción "No identificado" (9) se dará por concluido el registro y no se permitirá el llenado de ninguna otra plantilla.
    ...
    //ReglaDMED Favor de verificar variable CATFISCALIAID-IdentificadorFiscalia en su codigo diferente de 1 a 33 ya que la variable es de prioridad 1
    ...
    //ReglaDMED Favor de verificar variable CATMEDCONOCHECHOSID-Medio de conocimiento de los hechos en su codigo diferente de 1 a 8 ya que la variable es de prioridad 1
    ...
    //ReglaDMED. Favor de verificar variable NOMBREFISREGIOESPECIALIZADA-Nombre de la fiscalía regional o especializada ya que la variable es de prioridad 1 y no puede ser null
    ...
    //ReglaDMED. Favor de verificar variable NOMBREAGEMINISTERIOPUBLICO-Nombre y número de agencia del ministerio público ya que la variable es de prioridad 1 y no puede ser null
    ...
    //ReglaDMED. Favor de verificar variable FECHAPRESDENUNQUERELLA-Fecha de presentación de la denuncia o querella  ya que la variable es de prioridad 1 y no puede venir null
    ...
    //ReglaDMED. Favor de verificar variable HORAREPREDENUNQUERELLA-Hora de presentación de la denuncia o querella  ya que la variable es de prioridad 1 y no puede venir null
    ...
    //ReglaDMED. Favor de verificar variable INICIOCARPETAINVESTIGACION-Inicio carpeta de investigacion? ya que existe desglose en plantilla CarpetaInvestigacion 
    ...
    //ReglaDMED. La variable INICIOCARPETAINVESTIGACION es obligatoria, por lo tanto debe tener dato válido (1.Sí o 2.No)
    ...
    //ReglaDMED. La variable INICIOCARPETAINVESTIGACION debería ser Sí(1) ya que la variable CARPETAID tiene un ID válido.
    ...
    //ReglaDMED. Se indicó que existe una carpeta de investigación para la noticia criminal ( Es decir  la variable INICIOCARPETAINVESTIGACION= 1.Sí), sin embargo no se encontró el desglose en la tabla SENAP_CARPETAINVESTIGACION
